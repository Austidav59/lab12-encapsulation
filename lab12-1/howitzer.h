...
public:
    ...
    void setElevation(double angle) { elevation.setNaturalRadians(angle); } // Set elevation angle in radians
    ...
    void raise(double delta)
    {
        ...
        if (radians >= 0.0) {
            // For positive elevations
            if (delta < 0.0) {
                radians += fabs(delta); // Lowering elevation moves closer to 0
            }
            else {
                radians -= delta; // Raising elevation moves away from 0
            }
        }
        else {
            // For negative elevations
            if (delta < 0.0) {
                radians -= fabs(delta); // Lowering elevation moves further negative
            }
            else {
                radians += delta; // Raising elevation moves closer to zero
            }
        }

        elevation.setNaturalRadians(radians);
    }
    ...
    void rotate(double deltaAngle)
    {
        ...
        // Handle wrap-around logic for rotation
        if (newAngle > 2 * M_PI) {
            newAngle -= 2 * M_PI;
        }
        else if (newAngle < 0) {
